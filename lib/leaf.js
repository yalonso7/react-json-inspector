...
var Leaf = createClass({
    ...
    // childrenRef will store all of the root's children ref's for the ability to expand/collapse them
    childrenRef:{},
    onExpandCollpase: function(expanded) {
      this.setState({ expanded: expanded });
    },
    onExpandCollapseAll: function(expand) {
      if (this.childrenRef) {
        Object.keys(this.childrenRef)
          .forEach(function (k) {
             if (this.childrenRef[k]) {
               this.childrenRef[k].onExpandCollpase(expand);
             }
          }.bind(this));
      }
    },
    isLeaf: function(dataType){
        return (dataType !== 'Array' && dataType !== 'Object')
    },
    render: function() {
        var id = 'id_' + uid();
        var p = this.props;
        ...
        return React.createElement('div', { className: this.getClassName(), id: 'leaf-' + this._rootPath() },
          React.createElement('input', { className: 'json-inspector__radio', type: 'radio', name: p.id, id: id, tabIndex: -1 }),
          React.createElement('label', { className: 'json-inspector__line', htmlFor: id, onClick: this.toggle },
            React.createElement('div', { className: 'json-inspector__flatpath' },
              d.path),
            this.isLeaf(dataType) ?
              null :
              this.state.expanded ? React.createElement('i', { className: 'icon caret down' }) : React.createElement('i', { className: 'icon caret right' }),
            React.createElement('span', { className: this.isLeaf(dataType) ? 'json-inspector__leafKey' : 'json-inspector__key',
                onClick: this.isLeaf(dataType) ? onLabelClick : undefined },
              this.format(d.key),
              ':',
    ...
    renderTitle: function(onValueClick) {
        ...
        switch (t) {
            case 'Array':
                return React.createElement('span', { className: 'json-inspector__value json-inspector__value_helper' },
                  '[] ' + items(data.length));
            case 'Object':
                return React.createElement('span', { className: 'json-inspector__value json-inspector__value_helper' },
                  '{} ' + items(Object.keys(data).length));
            default:
                return React.createElement('span', { className: 'json-inspector__value json-inspector__value_' + t.toLowerCase(), onClick: onValueClick },
            ...
            return Object.keys(data).map(function(key) {
                ...
                return leaf({
                    ref: function(instance) {
                        if (p.root && !this.isLeaf(type(data))) {
                            this.childrenRef[key] = instance;
                        }
                    }.bind(this) ,
                    data: value,
                    label: key,
                    DTPath: this.nextDTPath(key, data),
                    prefix: childPrefix,
                    onClick: p.onClick,
                    id: p.id,
                    query: p.query,
    ...
    nextDTPath: function(key, data){
      var dataType = type(data);
      var currentDTPath = (this.props.DTPath) ? this.props.DTPath + '.' : '';
      if (dataType === 'Array') {
        return currentDTPath + '[' + key + ']';
      }
      return currentDTPath + '' + key;
    },
    data: function() {
    ...
    format: function(string) {
        return highlighter({
            ...
            highlightRenderer: this.props.highlightRenderer,
            fullKey: this.keypath()
        });
    },
